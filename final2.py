...
class Application:
    def __init__(self):
        ...
        self.panel2 = tk.Label(self.root, bg=style['bg_main'], relief=tk.RAISED, borderwidth=2)
        self.panel2.place(x=info_panel_x, y=550, width=250, height=250)
        self.panel2.config(highlightthickness=2, highlightbackground=style['border_color'])

        # Suggestions Label
        self.T4 = tk.Label(self.root, text="Suggestions:", 
                           fg='#E74C3C', 
                           font=label_font, 
                           bg='#F0F4F8')
        self.T4.place(x=info_panel_x, y=650)
        ...
        self.bt1.place(x=info_panel_x, y=700, width=250, height=50)

        self.bt2 = tk.Button(self.root, **button_style, command=self.action2)
        self.bt2.place(x=info_panel_x, y=760, width=250, height=50)

        self.bt3 = tk.Button(self.root, **button_style, command=self.action3)
        self.bt3.place(x=info_panel_x, y=820, width=250, height=50)
        ...
        if ok:
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    hand_crop_resized = cv2.resize(hand_crop, (128, 128))
                    hand_crop_gray = cv2.cvtColor(hand_crop_resized, cv2.COLOR_BGR2GRAY)
                    _, hand_crop_binary = cv2.threshold(hand_crop_gray, 127, 255, cv2.THRESH_BINARY_INV)
                    
                    # Predict
                    self.predict(hand_crop_binary)
            ...
            # Process binary image for display
            if self.hand_bbox is not None:
                self.current_image2 = Image.fromarray(hand_crop_binary)
